...
- 浏览器翻译字幕：支持浏览器字幕捕获、浏览器播放进度同步、翻译 API 配置和字幕输出。
...
```powershell
dotnet publish HaloPixelToolBox/HaloPixelToolBox/HaloPixelToolBox.csproj -c Release -p:Platform=x64 -p:PublishProfile= -r win-x64 --self-contained false -o release/v0.6.0-beta.1/HaloPixelToolBox-v0.6.0-beta.1-win-x64
```
