...
- 使用 `Directory.Build.props` 统一 `Version`、`AssemblyVersion`、`FileVersion`、`InformationalVersion`，当前统一为 `2.0.0`。
...
推荐使用发布脚本生成 2.0.0 的 x64 安装包与便携包：
...
1. 发布 WinUI 主程序到 `release/v2.0.0/HaloPixelToolBox-v2.0.0-win-x64/`。
2. 将主程序发布目录压缩为 `HaloPixelToolBox.Installer/Resources/Resource/Source.zip`，嵌入 WPF 安装器。
3. 发布 WPF 安装器到 `release/v2.0.0/HaloPixelToolBox.Installer-v2.0.0-win-x64/`。
4. 将安装器发布目录压缩为 `HaloPixelToolBox.Installer.Package/Source.zip`，嵌入自解压启动器。
5. 发布最终自解压包，并复制为 `release/v2.0.0/HaloPixelToolBox-v2.0.0-installer-win-x64.exe`。
6. 额外生成 `HaloPixelToolBox-v2.0.0-win-x64.zip` 便携包和 `SHA256SUMS.txt` 校验文件。

`Source.zip` 与 `release/` 都是构建产物，不提交到 Git；发布时作为 GitHub Release 附件上传。

## GitHub Release

2.0.0 发布建议从仓库默认主分支 `master` 打标签：

```powershell
gh release create v2.0.0 `
  release/v2.0.0/HaloPixelToolBox-v2.0.0-installer-win-x64.exe `
  release/v2.0.0/HaloPixelToolBox-v2.0.0-win-x64.zip `
  release/v2.0.0/SHA256SUMS.txt `
  --target master `
  --title "HaloPixelToolBox v2.0.0" `
  --notes "2.0.0 release"
```
